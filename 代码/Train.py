...
trainset = resources.read_relations("conll16st-en-zh-dev-train-test_LDC2016E50/conll16st-zh-01-08-2016-train/",
                                    ignore_types=["Explicit", "AltLex"], partial_sampling=True)
devset = resources.read_relations("conll16st-en-zh-dev-train-test_LDC2016E50/conll16st-zh-01-08-2016-dev/",
                                  ignore_types=["Explicit", "AltLex"], partial_sampling=True)
testset = resources.read_relations(
    "conll16st-en-zh-dev-train-test_LDC2016E50/conll16st-zh-01-08-2016-test/", ignore_types=["Explicit", "AltLex"])
"""
...
for nexp in range(5):
    ...
    test_senses = resources.read_senses(
        "conll16st-en-zh-dev-train-test_LDC2016E50/conll16st-zh-01-08-2016-test/", ignore_types=["Explicit", "AltLex"])
    ...
    for epoch in range(25):
        _ = model.fit(X, y, nb_epoch=epoch+1, batch_size=batch_size, verbose=1  # validation_data=(dev_X, dev_y)
                      , initial_epoch=epoch
                      )
        #scores = model.evaluate(X, y, verbose=1)
        print("\tdev\ttest")
        dev_scores = model.evaluate(
            dev_X, dev_y, verbose=0, batch_size=batch_size)
        print("acc.\t%.2f%%" % (dev_scores[1]*100), end="", flush=True)
        test_scores = model.evaluate(
            test_X, test_y, verbose=0, batch_size=batch_size)
        print("\t%.2f%%" % (test_scores[1]*100), end="", flush=True)
        if dev_scores[1] > best_val:
            print("\t*")
            best_val_test_any = eval_any(test_labels, [np.argmax(
                y) for y in model.predict(test_X, batch_size=batch_size)])
            best_val = dev_scores[1]
            best_val_test = (best_val_test_any,)
            print("Test accuracy on any sense:",
                  round(best_val_test_any*100, 2))
            #print("Saving model...")
