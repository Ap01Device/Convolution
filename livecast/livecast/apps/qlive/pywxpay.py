...
class WXPayUtil(object):
    ...
    @staticmethod
    def lxml2dict(xml_str):
        try:
            tree = etree.fromstring(xml_str, etree.XMLParser(resolve_entities=False))
        except:
            return {}
        else:
...
class WXPay(object):
    ...
    def fill_request_data(self, data, withdraw=False):
        ...
        new_data_dict = copy.deepcopy(data)
        if withdraw == True:
            new_data_dict['mch_appid'] = self.app_id
            new_data_dict['mchid'] = self.mch_id
        else:
    ...
    def process_response_xml(self, resp_xml, withdraw=False):
        """ 处理微信支付返回的 xml 格式数据
    ...
    def withdraw(self, data, timeout=None):
        """提现
        :param data: dict
        :param timeout: int
        :return: dict
        """
        _timeout = self.timeout if timeout is None else timeout
        url = WXPayConstants.WITHDRAW_URL
        resp_xml = self.request_with_cert(url, self.fill_request_data(data, True), _timeout)
        return self.process_response_xml(resp_xml, True)
