...
class TLSSigAPI:
    ...
    def __check_field(self, m):
        if m["TLS.identifier"] != self.__identifier:
            self._err_msg = "identifier %s in req and identifier %s in tlssig not match" % (
            self.__identifier, m["TLS.identifier"])
            return -1
        if atoi(m["TLS.sdk_appid"]) != self.__sdkappid:
            self._err_msg = "sdkappid %d in req and identifier %s in tlssig not match" % (
            self.__identifier, m["TLS.sdk_appid"])
            return -2
        current_time = time.time()
        if atoi(m["TLS.expire_after"]) + atoi(m["TLS.time"]) < current_time:
            self._err_msg = "tls sig expired expire %s add init time %s lower than current time %d" % (
            m["TLS.expire_after"], m["TLS.time"], current_time)
            return -3,
    ...
    def tls_gen_sig(self, identifier):
        ...
        sig_field = OpenSSL.crypto.sign(pk_loaded, fix_str.encode('utf8'), "sha256")
